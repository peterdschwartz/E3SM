  ...
  subroutine FireArea (bounds, &
    ...
    associate(                                                                &
     ...
     do fc = 1,num_soilc
        ...
        do p = col_pp%pfti(c), col_pp%pftf(c)
              ...
              if( (crop(veg_pp%itype(p)) == 0 .and. .not. iscft(veg_pp%itype(p))) .and. &
                  cropf_col(c)  <  1.0_r8 .and. veg_pp%wtcol(p)>0._r8 )then
      ...
      if (transient_landcover) then    !true when landuse data is used
         ...
         do fc = 1,num_soilc
            ...
            do p = col_pp%pfti(c), col_pp%pftf(c)
               ! For non-crop -- natural vegetation and bare-soil
               if((crop(veg_pp%itype(p)) == 0 .and. .not. iscft(veg_pp%itype(p))) .and. &
                  cropf_col(c)  <  1.0_r8 .and. veg_pp%wtcol(p) )then
                  if( needleleaf(veg_pp%itype(p)) == 0 .and. &
                       climatezone(veg_pp%itype(p)) == 1 .and. &
     ...
     do fc = 1,num_soilc
        ...
        do p = col_pp%pfti(c),col_pp%pftf(c)
           ! For crop
           if( forc_t(t)  >=  SHR_CONST_TKFRZ .and. (crop(veg_pp%itype(p)) == 1 .or. iscft(veg_pp%itype(p))) .and.  &
 ...
 subroutine FireFluxes (num_soilc, filter_soilc, num_soilp, filter_soilp, &
   ...
   associate(                                                                                                   &
     ...
     do fp = 1,num_soilp
        ...
        if( (crop(itype) == 0 .and. .not. iscft(itype)) .and. &
