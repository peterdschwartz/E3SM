  ...
  subroutine AllocationInit (bounds, elm_fates)
    ...
    if (nu_com .eq. 'ECA' .or. nu_com .eq. 'MIC') then
       if (.not.use_fates) then
          allocate(ft_index_bigleaf(bounds%begp:bounds%endp)); ft_index_bigleaf(:) = -1
          allocate(veg_rootc_bigleaf(bounds%begp:bounds%endp,1:nlevdecomp));veg_rootc_bigleaf(:,:) = spval
       end if
    end if
