...
contains
   ...
   subroutine SurfaceAlbedo(bounds, &
      ...
      real(r8),parameter :: extkn = 0.30_r8       ! nitrogen allocation coefficient
      ...
      real(r8) :: blai             ! lai buried by snow: tlai - elai
      real(r8) :: bsai             ! sai buried by snow: tsai - esai
      ...
      real(r8) :: rho(1:num_nourbanp, numrad)        ! leaf/stem refl weighted by fraction LAI and SAI
      real(r8) :: tau(1:num_nourbanp, numrad)        ! leaf/stem tran weighted by fraction LAI and SAI
      ...
      associate ( &
         ...
         do c = bounds%begc, bounds%endc
            albsfc(c, :) = albsoi(c, :)
            h2osno_liq(c, -nlevsno + 1:0) = h2osoi_liq(c, -nlevsno + 1:0)
         ...
         if (DO_SNO_AER) then
            do i = -nlevsno + 1, 0
               do fc = 1, num_nourbanc
                  c = filter_nourbanc(fc)
                  mss_cnc_aer_in_fdb(c, i, 1) = mss_cnc_bcphi(c, i)
                  mss_cnc_aer_in_fdb(c, i, 2) = mss_cnc_bcpho(c, i)
                  ...
                  if (DO_SNO_OC) then
                     mss_cnc_aer_in_fdb(c, i, 3) = mss_cnc_ocphi(c, i)
                     mss_cnc_aer_in_fdb(c, i, 4) = mss_cnc_ocpho(c, i)
                  end if

                  mss_cnc_aer_in_fdb(c, i, 5) = mss_cnc_dst1(c, i)
                  mss_cnc_aer_in_fdb(c, i, 6) = mss_cnc_dst2(c, i)
                  mss_cnc_aer_in_fdb(c, i, 7) = mss_cnc_dst3(c, i)
                  mss_cnc_aer_in_fdb(c, i, 8) = mss_cnc_dst4(c, i)
               end do
            end do
         end if
         ...
         if (use_snicar_frc) then
            ...
            do i = -nlevsno + 1, 0
               do fc = 1, num_nourbanc
                  mss_cnc_aer_in_frc_bc(c, i, 5) = mss_cnc_dst1(c, i)
                  mss_cnc_aer_in_frc_bc(c, i, 6) = mss_cnc_dst2(c, i)
                  mss_cnc_aer_in_frc_bc(c, i, 7) = mss_cnc_dst3(c, i)
                  mss_cnc_aer_in_frc_bc(c, i, 8) = mss_cnc_dst4(c, i)
                  if (DO_SNO_OC) then
                     mss_cnc_aer_in_frc_bc(c, i, 3) = mss_cnc_ocphi(c, i)
                     mss_cnc_aer_in_frc_bc(c, i, 4) = mss_cnc_ocpho(c, i)
                  end if
               end do
            end do
            ...
            if (DO_SNO_OC) then
               ...
               do i = -nlevsno + 1, 0
                  do fc = 1, num_nourbanc
                     mss_cnc_aer_in_frc_oc(c, i, 1) = mss_cnc_bcphi(c, i)
                     mss_cnc_aer_in_frc_oc(c, i, 2) = mss_cnc_bcpho(c, i)
                     mss_cnc_aer_in_frc_oc(c, i, 5) = mss_cnc_dst1(c, i)
                     mss_cnc_aer_in_frc_oc(c, i, 6) = mss_cnc_dst2(c, i)
                     mss_cnc_aer_in_frc_oc(c, i, 7) = mss_cnc_dst3(c, i)
                     mss_cnc_aer_in_frc_oc(c, i, 8) = mss_cnc_dst4(c, i)
                  end do
               end do
            ...
            do i = -nlevsno + 1, 0
               do fc = 1, num_nourbanc
                  mss_cnc_aer_in_frc_dst(c, i, 1) = mss_cnc_bcphi(c, i)
                  mss_cnc_aer_in_frc_dst(c, i, 2) = mss_cnc_bcpho(c, i)
                  if (DO_SNO_OC) then
                     mss_cnc_aer_in_frc_dst(c, i, 3) = mss_cnc_ocphi(c, i)
                     mss_cnc_aer_in_frc_dst(c, i, 4) = mss_cnc_ocpho(c, i)
                  end if
               end do
            end do
         ...
         do ib = 1, numrad
            do fp = 1, num_vegsol
               p = filter_vegsol(fp)
               wl = elai(p)/max(elai(p) + esai(p), mpe)
               ws = esai(p)/max(elai(p) + esai(p), mpe)
               rho(fp, ib) = max(rhol(veg_pp%itype(p), ib)*wl + rhos(veg_pp%itype(p), ib)*ws, mpe)
               tau(fp, ib) = max(taul(veg_pp%itype(p), ib)*wl + taus(veg_pp%itype(p), ib)*ws, mpe)
            end do
         end do
         ...
         do fp = 1, num_nourbanp
            ...
            if (nlevcan > 1) then
               blai = tlai(p) - elai(p)
               bsai = tsai(p) - esai(p)
               if (blai + bsai == 0._r8) then
                  ncan(p) = nrad(p)
               else
                  dincmax_sum = 0._r8
                  do iv = nrad(p) + 1, nlevcan
                     dincmax_sum = dincmax_sum + dincmax
                     if (((blai + bsai) - dincmax_sum) > 1.e-06_r8) then
                        ncan(p) = iv
                        dinc = dincmax
               ...
               if (abs(laisum - tlai(p)) > 1.e-06_r8 .or. abs(saisum - tsai(p)) > 1.e-06_r8) then
                  ...
                  write (iulog, *) tlai(p), elai(p), blai, laisum, tsai(p), esai(p), bsai, saisum
                  call endrun(decomp_index=p, elmlevel=namep, msg=errmsg(__FILE__, __LINE__))
#endif
               end if
            end if
   ...
   subroutine TwoStream(bounds, &
      ...
      real(r8), intent(in)  :: rho(1:, 1:) ! leaf/stem refl weighted by fraction LAI and SAI [pft, numrad]
      real(r8), intent(in)  :: tau(1:, 1:) ! leaf/stem tran weighted by fraction LAI and SAI [pft, numrad]
      type(canopystate_type), intent(in)    :: canopystate_vars
      type(surfalb_type), intent(inout) :: surfalb_vars
      !
      ! !LOCAL VARIABLES:
      ...
      real(r8) :: gdir             ! leaf projection in solar direction (0 to 1)
      real(r8) :: twostext         ! optical depth of direct beam per unit leaf area
      real(r8) :: avmu             ! average diffuse optical depth
      real(r8) :: omega            ! fraction of intercepted radiation that is scattered (0 to 1)
      ...
      associate ( &
         ...
         do ib = 1, numrad
            do fp = 1, num_vegsol
               ...
               chil = min(max(xl(veg_pp%itype(p)), -0.4_r8), 0.6_r8)
               if (abs(chil) <= 0.01_r8) chil = 0.01_r8
               phi1 = 0.5_r8 - 0.633_r8*chil - 0.330_r8*chil*chil
               phi2 = 0.877_r8*(1._r8 - 2._r8*phi1)
               gdir = phi1 + phi2*cosz
               twostext = gdir/cosz
               avmu = (1._r8 - phi1/phi2*log((phi1 + phi2)/phi1))/phi2
               temp0 = gdir + phi2*cosz
               temp1 = phi1*cosz
               temp2 = (1._r8 - temp1/temp0*log((temp1 + temp0)/temp1))
               ...
               omegal = rho(fp, ib) + tau(fp, ib)
               asu = 0.5_r8*omegal*gdir/temp0*temp2
               betadl = (1._r8 + avmu*twostext)/(omegal*avmu*twostext)*asu
               betail = 0.5_r8*((rho(fp, ib) + tau(fp, ib)) + (rho(fp, ib) - tau(fp, ib)) &
                                *((1._r8 + chil)/2._r8)**2)/omegal
               ...
               b = 1._r8 - omega + omega*betai
               c1 = omega*betai
               tmp0 = avmu*twostext
               d = tmp0*omega*betad
               f = tmp0*omega*(1._r8 - betad)
               tmp1 = b*b - c1*c1
               h = sqrt(tmp1)/avmu
               ...
               s1 = exp(-t1)
               t1 = min(twostext*(elai(p) + esai(p)), 40._r8)
               s2 = exp(-t1)
               ...
               a1 = h1/sigma*(1._r8 - s2*s2)/(2._r8*twostext) &
                    + h2*(1._r8 - s2*s1)/(twostext + h) &
                    + h3*(1._r8 - s2/s1)/(twostext - h)

               a2 = h4/sigma*(1._r8 - s2*s2)/(2._r8*twostext) &
                    + h5*(1._r8 - s2*s1)/(twostext + h) &
                    + h6*(1._r8 - s2/s1)/(twostext - h)

               fabd_sun(p, ib) = (1._r8 - omega)*(1._r8 - s2 + 1._r8/avmu*(a1 + a2))
               ...
               a1 = h7*(1._r8 - s2*s1)/(twostext + h) + h8*(1._r8 - s2/s1)/(twostext - h)
               a2 = h9*(1._r8 - s2*s1)/(twostext + h) + h10*(1._r8 - s2/s1)/(twostext - h)

               fabi_sun(p, ib) = (1._r8 - omega)/avmu*(a1 + a2)
                  ...
                  if (nlevcan == 1) then
                     ...
                     do iv = 1, nrad(p)
                        ...
                        da1 = h1/sigma*s2*s2 + h2*s2*s1 + h3*s2/s1 &
                              + (1._r8 - s2*s1)/(twostext + h)*dh2 &
                              + (1._r8 - s2/s1)/(twostext - h)*dh3
                        da2 = h4/sigma*s2*s2 + h5*s2*s1 + h6*s2/s1 &
                              + (1._r8 - s2*s1)/(twostext + h)*dh5 &
                              + (1._r8 - s2/s1)/(twostext - h)*dh6

                        ! Flux derivatives

                        d_ftid = -twostext*h4/sigma*s2 - h*h5*s1 + h*h6/s1 + dh5*s1 + dh6/s1
                        d_fabd = -(dh2 + dh3) + (1._r8 - albgrd(c, ib))*twostext*s2 - (1._r8 - albgri(c, ib))*d_ftid
                        d_fabd_sun = (1._r8 - omega)*(twostext*s2 + 1._r8/avmu*(da1 + da2))
                        ...
                        da1 = h7*s2*s1 + h8*s2/s1 + (1._r8 - s2*s1)/(twostext + h)*dh7 + (1._r8 - s2/s1)/(twostext - h)*dh8
                        da2 = h9*s2*s1 + h10*s2/s1 + (1._r8 - s2*s1)/(twostext + h)*dh9 + (1._r8 - s2/s1)/(twostext - h)*dh10

                        ! Flux derivatives

                        d_ftii = -h*h9*s1 + h*h10/s1 + dh9*s1 + dh10/s1
                        d_fabi = -(dh7 + dh8) - (1._r8 - albgri(c, ib))*d_ftii
                        d_fabi_sun = (1._r8 - omega)/avmu*(da1 + da2)
